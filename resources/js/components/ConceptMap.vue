        ...
        <div class="concept-map-container" ref="mapContainer">
            ...
            <div class="concept-map-canvas" ref="canvas">
                ...
                <svg class="edges-svg" :width="canvasWidth" :height="canvasHeight"
                    ...
                    <line v-for="edge in localEdges" :key="edge.id" :x1="getNodeCenter(edge.source).x"
                        :y1="getNodeCenter(edge.source).y" :x2="getNodeCenter(edge.target).x"
                        :y2="getNodeCenter(edge.target).y" stroke="#6b7280" stroke-width="2" stroke-dasharray="5,5"
                        marker-end="url(#arrowhead)" />
                    <text v-for="edge in localEdges" :key="'label-' + edge.id" :x="(getNodeCenter(edge.source).x + getNodeCenter(edge.target).x) / 2
                        " :y="(getNodeCenter(edge.source).y + getNodeCenter(edge.target).y) /
                            2 -
                            5
                            " text-anchor="middle" class="edge-label" @dblclick="editEdge(edge)">
                        {{ edge.label || "relates to" }}
                    </text>
                </svg>

                <!-- Nodes -->
                <div v-for="node in localNodes" :key="node.id" :id="node.id" :style="{
                    left: node.position.x + 'px',
                    top: node.position.y + 'px',
                    width: (node.style?.width || '120px') + ' !important',
                    height: (node.style?.height || '80px') + ' !important',
                    backgroundColor: (node.style?.background || '#FF4757') + ' !important',
                    fontSize: (node.style?.fontSize || '14px') + ' !important',
                    color: (node.style?.color || '#fff') + ' !important',
                }" class="concept-node" @mousedown="startDrag(node, $event)" @dblclick="editNode(node)">
            ...
            <div class="concept-map-status">
                <div class="flex items-center space-x-2">
                    <div class="w-2 h-2 rounded-full" :class="saving ? 'bg-yellow-400 animate-pulse' : 'bg-green-400'">
                    </div>
            ...
            <div class="modal-content" @click.stop>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input v-model="editingNode.data.label" class="form-input" type="text" />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input v-model="editingNode.style.background" type="color" class="form-input" />
                    </div>
                </div>
            ...
            <div class="modal-content" @click.stop>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input v-model="editingEdge.label" class="form-input" type="text"
...
export default {
    ...
    setup(props) {
        ...
        const loadData = () => {
            try {
                console.log("🔥 Loading data from props:", props.nodes, props.edges);

                // Pastikan nodes adalah array
                if (props.nodes && Array.isArray(props.nodes)) {
                    localNodes.value = props.nodes.map((node) => {
                        ...
                        const newNode = {
                            ...
                            data: node.data || {
                                label: node.label || node.data?.label || 'Unknown',
                                frequency: node.frequency || node.data?.frequency || 1,
                                category: node.category || node.data?.category || 'detail'
                            },
                            style: node.style || {
                                background: node.color || node.data?.color || '#4ECDC4',
                                width: node.width || '80px',
                                height: node.height || '60px',
                                fontSize: node.fontSize || '12px'
                            }
                        };

                        console.log("📝 Processing node:", newNode);
                        return newNode;
                    });
                } else {
                    console.warn("⚠️ Nodes prop is not an array:", props.nodes);
                    localNodes.value = [];
                }

                // Pastikan edges adalah array
                if (props.edges && Array.isArray(props.edges)) {
                    localEdges.value = props.edges.map((edge) => {
                        ...
                        console.log("📝 Processing edge:", newEdge);
                        return newEdge;
                    });
                } else {
                    console.warn("⚠️ Edges prop is not an array:", props.edges);
                    localEdges.value = [];
                }

                console.log("✅ Processed nodes:", localNodes.value);
                console.log("✅ Processed edges:", localEdges.value);
        ...
        onMounted(() => {
            ...
            nextTick(() => {
                if (canvas.value) {
                    ...
                    console.log("📐 Canvas size:", canvasWidth.value, canvasHeight.value);
                }
        ...
        watch(
            () => props.nodes,
            (newNodes) => {
                console.log("🔄 Nodes prop changed");
                loadData();
            },
            { deep: true }
        );
        ...
        watch(
            () => props.edges,
            (newEdges) => {
                console.log("🔄 Edges prop changed");
                loadData();
            },
            { deep: true }
        );
        ...
        const autoLayout = () => {
            ...
            const root = localNodes.value[0];
            const centerX = 400;
            const startY = 50;
            root.position = { x: centerX, y: startY };

            const children = localNodes.value.slice(1);

            const spacingX = 180;  // jarak horizontal antar node
            const spacingY = 150;  // jarak vertikal antar row
            const maxWidth = 800;  // batas lebar

            let currentX = 0;
            let currentY = startY + 200; // mulai dari bawah root
            let row = [];

            children.forEach((node, index) => {
                // kalau sudah melewati maxWidth, pindah baris
                if (currentX + spacingX > maxWidth) {
                    currentX = 0;
                    currentY += spacingY;
                }

                node.position = {
                    x: currentX + (centerX - maxWidth / 2), // biar center
                    y: currentY,
                };
        ...
        const saveChanges = async () => {
            ...
            try {
                const response = await axios.put(props.autosaveUrl, {
                    nodes: localNodes.value,
                    edges: localEdges.value,
                });

                console.log("➡️ Autosave URL:", props.autosaveUrl);
                console.log("➡️ Nodes:", JSON.stringify(localNodes.value));
                console.log("➡️ Edges:", JSON.stringify(localEdges.value));
